#include "CircleObject_phys.h"
#include "BoxObject.h"

#include <cmath>

CircleObject_phys::CircleObject_phys(GameManager* game)  // Default Debug Constructor
    : GameObject(game), radius(25), shape(sf::CircleShape(25)), colour(sf::Color::Red) {
    shape.setPosition(pos.x - radius, pos.y - radius);
}

CircleObject_phys::CircleObject_phys(GameManager* game, Vector2 pos, float radius, sf::Color colour)
...
void CircleObject_phys::drawObject() const {
    game->window.draw(shape);
}

void CircleObject_phys::SetPosition(Vector2 newPos) {
    pos.x = newPos.x;  // 'pos' is inherited from GameObject
    pos.y = newPos.y;
    shape.setPosition(pos.x - radius, pos.y - radius);
}

std::string CircleObject_phys::toString() const {
    return "CircleObject_phys";
}

void CircleObject_phys::update(float gametime) {
...
bool CircleObject_phys::isColliding(GameObject* other) {
    if (dynamic_cast<CircleObject_phys*>(other))  // CircleObject_phys -> CircleObject_phys Collision
    {
        CircleObject_phys* otherball = (CircleObject_phys*)other;  // cast to CircleObject_phys
    ...
    } else if (dynamic_cast<BoxObject*>(other))  // CircleObject_phys -> BoxObject Collision
    {
...
void CircleObject_phys::resolveCollision(GameObject* other) {
    float distance = std::sqrt(pos.x - other->pos.x * pos.x - other->pos.x + pos.y - other->pos.y * pos.y - other->pos.y);

    if (dynamic_cast<CircleObject_phys*>(other)) {  // Ball vs Ball

        CircleObject_phys* otherBall = (CircleObject_phys*)other;
        ...
        std::cout << "COLLISION ERROR: OTHER TYPE NOT BOXOBJECT OR CIRCLEOBJECT\n";
    }
}
